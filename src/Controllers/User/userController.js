...
userController.favToggle = async(item, userId) =>{
  ...
  let favorites = !findUser.dataValues.favorites ? [] : findUser.dataValues.favorites ? typeof findUser.dataValues.favorites === "string" ? findUser.dataValues.favorites === "[]" ? [] : (typeof findUser.dataValues.favorites === "string" && findUser.dataValues.favorites !== "[]") ? JSON.parse(findUser.dataValues.favorites) : findUser.dataValues.favorites : findUser.dataValues.favorites : findUser.dataValues.favorites
...
userController.cartToggle = async(item, userId) =>{
  ...
  if(!cart.length){
    ...
    if(findItem){
      ...
      await findUser.save();
      return {msg: "Item removed from cart", data: findUser.dataValues, product: item.id};
    }else if(!findItem){
      // Si el item no existe lo agrega
      cart.push(item);
      await findUser.update({
      ...
      await findUser.save();
      return {msg: "Item added to cart", data: findUser.dataValues};
    }
  };
...
userController.contactPreference = async(data) =>{
  try{
    ...
    return {msg: "Preferences updated", data: {...findUser.dataValues, cart: findUser.dataValues.cart ? findUser.dataValues.cart : []}};
